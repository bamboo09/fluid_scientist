...
平台同时支持两类 `ExecutionTarget`，用户在实验设计或 Gate 2 审批时选择：

1. `workstation_openfoam`：通过 SSH 连接已安装 OpenFOAM 的工作站，直接运行算例，不经过 Slurm。
2. `hpc_slurm`：通过数据节点发布制品、Login 节点提交 Slurm、计算节点运行 OpenFOAM。

两者实现统一 `ExecutionTargetAdapter`：能力检查、制品发布、提交、状态、取消、结果回收和健康诊断。ResearchSpec 不绑定平台；ExperimentPlan 与 Case Manifest 记录已批准的 `execution_target_id`，更换平台需重新执行资源/软件能力检查并重新批准 Gate 2。

工作站不通过临时 SSH Shell 保持长任务。部署固定、版本化的 `fluid-worker`，只接受严格 Schema 和白名单 OpenFOAM 命令，生成本地 job ID、PID、状态文件、日志和结果清单。SSH 断开后任务继续运行；控制平面通过 `fluid-worker status/cancel/collect` 恢复管理。工作站配置包含候选主机、SSH 用户、known_hosts、身份文件、工作根目录、OpenFOAM 环境入口、CPU/内存/并发上限。真实地址和账号只存在于本地 Secret/环境配置，不进入 Git、报告、普通日志或 Skill。

提交前执行目标能力发现：网络/SSH、`foamVersion`、白名单命令、CPU、可用内存、磁盘空间、工作目录权限和 worker 协议版本。多个候选地址时只选择通过 host-key 校验且能力检查成功的地址；禁止仅按 ping 结果信任主机。
...
- `execution_targets`：统一执行目标协议、工作站 worker 适配器、HPC Slurm 适配器、能力发现与目标选择。
...
实验设计页显示执行平台选择：工作站 OpenFOAM 或 HPC Slurm。每个选项展示当前连通性、OpenFOAM 版本、可用资源、队列/并发限制、预计成本和能力检查时间。平台不可用时禁止批准 Gate 2，并显示具体缺失项；系统不得静默切换到另一平台。
