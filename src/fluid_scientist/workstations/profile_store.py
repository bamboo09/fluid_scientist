...
_DEFAULT_DB_DIR = os.path.join(
    os.environ.get("TEMP", os.environ.get("LOCALAPPDATA", os.path.expanduser("~"))),
    "fluid_scientist",
)
...
class WorkstationProfileStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        """Open a new short-lived connection with WAL journaling (fallback to DELETE)."""
        conn = sqlite3.connect(self._db_path, check_same_thread=False)
        try:
            conn.execute("PRAGMA journal_mode=WAL")
        except sqlite3.OperationalError:
            # WAL not supported on this filesystem — fall back to default
            pass
