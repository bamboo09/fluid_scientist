...
def _write_case_to_disk(compiled: dict[str, Any], case_dir: str) -> None:
    ...
    # Map filenames to their OpenFOAM field class for the FoamFile header
    _FOAM_CLASS_MAP = {
        "U": "volVectorField",
        "p": "volScalarField",
    }
    _BANNER = (
        ...
        "|  \\\\    /   O peration     | Version:  OpenFOAM-13                            |\n"
        "|   \\\\  /    A nd           | Web:      www.openfoam.org                       |\n"
        "|    \\/     M anipulation   |                                                 |\n"
        "\\*---------------------------------------------------------------------------*/\n"
    )

    def _foam_header_str(obj: str, cls: str = "dictionary") -> str:
        return (
            "FoamFile\n"
            "{\n"
            "    version     2.0;\n"
            ...
            for filename, content in section_content.items():
                ...
                with open(filepath, "w", encoding="utf-8", newline="\n") as f:
                    if isinstance(content, str):
                        # Pre-formatted OpenFOAM string (e.g. blockMeshDict)
                        f.write(_BANNER)
                        f.write(f"// Generated by Fluid Scientist\n\n")
                        cls = _FOAM_CLASS_MAP.get(filename, "dictionary")
                        f.write(_foam_header_str(filename, cls))
                        f.write("\n")
                        f.write(content)
                        f.write("\n")
