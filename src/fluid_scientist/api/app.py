...
def create_app(
    ...
    *,
    ...
    def list_execution_targets() -> tuple[TargetCapabilityStatus, ...]:
        targets = getattr(application.state, "execution_targets", configured_targets)
        return tuple(capability_cache.get(target) for target in targets)
    ...
    def configure_workstation(
        ...
        if not known_hosts_file:
            default_known_hosts = _Path.home() / ".ssh" / "known_hosts"
            if default_known_hosts.is_file():
                known_hosts_file = str(default_known_hosts)
